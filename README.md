...
- **时间设置**：
  ...
  - 短休息时长
  - 长休息时长
  - **长休息间隔**（每完成多少个番茄后触发长休息，默认 4 个）
- **声音设置**：启用声音提醒和音量调节
- **Bark 推送**：配置 iOS 推送通知
- **iCloud 同步**：查看同步状态（自动启用）
- **测试模式**：使用秒代替分钟进行快速测试

### iCloud 同步

所有设置和应用状态会自动通过 iCloud 在多台 Mac 设备间同步。
...
3. **自动同步**
   - 所有设置修改会立即同步到 iCloud
   - 其他设备会在几秒内收到更新
   - 即使离线，数据也会保存在本地，联网后自动同步

详细文档：[docs/icloud-sync.md](docs/icloud-sync.md)
